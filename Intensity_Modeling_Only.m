% % Intensity_Modeling_Only simulates the expected PSF's of a list of 
% celestial bodies through the input mask.

% It then adds together the PSF's to generate a simulation of the entire
% target system, and generates sample data as if it was collected by a 
% simulated camera. 
...
function [outname] = Intensity_Modeling_Only(configFile,EXPOSURE_NUMBER)
...
load(configFile);
    ...
    A_psuedoarea = (sum(sum(gray2d_SP)))/(size(gray2d_SP,1)*size(gray2d_SP,1));
    A_psuedoarea = A_psuedoarea * pi * ((Telescope_Diameter/2)^2);
    ...
    outname = (['SIM_TEMP_OUT_' num2str(EXPOSURE_NUMBER) '.mat']);
    save(outname);
end